...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "XXX" then
		library.WorkspaceName = convertfilename(windowName, "XXX")
	end
	local xxxLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = xxxLibrary, xxxLibrary
	...
	xxxLibrary.Name = "     "
	xxxLibrary.Parent = library.gui_parent
	xxxLibrary.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	xxxLibrary.DisplayOrder = 10
	xxxLibrary.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = xxxLibrary
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./XXX") then
						makefolder("./XXX")
					end
					local common_string = "./XXX/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./XXX/" .. tostring(custom_workspace or library.WorkspaceName)
						end
